...
fun getCodeForToolbarKey(key: ToolbarKey) = Settings.getInstance().getCustomToolbarKeyCode(key) ?: when (key) {
    ...
    PRAVKA_CLEAN -> KeyCode.PRAVKA_CLEAN
    PRAVKA_VOICE -> KeyCode.PRAVKA_VOICE
    PRAVKA_SHORTER -> KeyCode.PRAVKA_SHORTER
...
fun getCodeForToolbarKeyLongClick(key: ToolbarKey) = Settings.getInstance().getCustomToolbarLongpressCode(key) ?: when (key) {
    ...
    // Long press on the Pravka key stores the API key from the clipboard.
    PRAVKA_CLEAN -> KeyCode.PRAVKA_SET_KEY
...
enum class ToolbarKey {
    ...
    PRAVKA_CLEAN, PRAVKA_VOICE, PRAVKA_SHORTER, PRAVKA_LONGER, PRAVKA_POLISH
}
...
val defaultToolbarPref by lazy {
    // PravkaBoard: factory default mirrors the owner's Yandex-keyboard nav row
    // (|<  select-all  ^  <  select  >  v  copy  paste  >|), then the Pravka keys.
    val default = listOf(
        PAGE_START, SELECT_ALL, UP, LEFT, SELECT_WORD, RIGHT, DOWN, COPY, PASTE, PAGE_END,
        PRAVKA_VOICE, PRAVKA_POLISH, PRAVKA_CLEAN, SETTINGS, CLIPBOARD, UNDO, REDO, CUT, DPAD, SPLIT,
    )
